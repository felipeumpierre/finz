...
`/insights` is a single command with no sub-commands. Ignore any arguments — always run the full cockpit.
...
3. Compute all cross-domain figures as specified in SKILL.md:
   ...
   - Monthly cash flow (avg last 3 months from bank-state)
...
6. End with the next-steps prompt.
...
- `/insights` produces a single, complete response — no back-and-forth.
- Do not run deep domain analysis — the cockpit is a summary view. Refer the user to `/bank`, `/portfolio`, `/insurance`, or `/steuer` for depth.
